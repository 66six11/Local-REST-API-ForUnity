...
namespace LocalRestAPI
{
    public class ApiLogWindow : EditorWindow
    {
        ...
        private void DrawLogEntry(ApiLogEntry log)
        {
            ...
            // 基本信息行
            EditorGUILayout.BeginHorizontal();
            GUILayout.TextField($"[{log.Timestamp:HH:mm:ss.fff}]", GUILayout.Width(100));
            GUILayout.TextField(log.Type, GUILayout.Width(80));

            if (!string.IsNullOrEmpty(log.Method))
            {
                GUILayout.TextField(log.Method, GUILayout.Width(60));
            }
            ...
            if (!string.IsNullOrEmpty(log.Url))
            {
                ...
                GUILayout.TextField(displayUrl, GUILayout.ExpandWidth(true));
                GUI.contentColor = originalColor; // 恢复颜色
            }

            if (log.StatusCode != 0)
            {
                GUILayout.TextField(log.StatusCode.ToString(), GUILayout.Width(40));
            }

            if (log.DurationMs > 0)
            {
                GUILayout.TextField($"{log.DurationMs:F1}ms", GUILayout.Width(70));
            }
